        ...
        $('#search_product')
            .autocomplete({
                ...
                response: function (event, ui) {
                    if (ui.content.length === 1 && !ui.content[0].notfound) {
                        // hanya auto-select jika stok/izin memenuhi
                        ...
                        if (
                            (item.enable_stock == 1 && item.qty_available > 0) ||
                            item.enable_stock == 0 ||
                        ...
                        // tampilkan satu baris "Not found" (non-selectable)
                        ui.content.push({
                            id: 0,
                            text:
                                typeof LANG !== 'undefined' && LANG.no_products_found
                ...
                focus: function (event, ui) {
                    // cegah fokus pada item dummy atau item out-of-stock (sesuai logika lama)
                    if ((ui.item && ui.item.notfound) || (ui.item && ui.item.qty_available <= 0)) {
                        event.preventDefault();
                        return false;
                    }
                },
                select: function (event, ui) {
                    // cegah pemilihan item dummy
                    if (ui.item && ui.item.notfound) {
                        event.preventDefault();
                        return false;
                    }
                    ...
                    if (
                        ...
                        // Pre select lot number hanya jika term sama dengan lot_number
            ...
            // render baris "Not found"
            if (item.notfound) {
                return $('<li>')
                    .addClass('ui-state-disabled')
                    .append('<div class="text-muted">' + item.text + '</div>')
                    .appendTo(ul);
            }
            ...
            if (
                ...
                var selling_price = item.variation_group_price
                    ? item.variation_group_price
                    : item.selling_price;
...
function set_location() {
    ...
    if ($('input#location_id').val()) {
        $('input#search_product').prop('disabled', false);
        if (!$('#__is_mobile').length) {
